...
<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 py-4 md:py-8 pb-8">
    <div class="container mx-auto px-4 max-w-7xl">
      <!-- Loading -->
      <div v-if="isLoadingLeague" class="flex items-center justify-center py-20">
      ...
      <template v-else-if="leagueDetailStore.isDraftNotStarted">
        ...
        >
          <v-icon name="hi-solid-lightning-bolt" class="w-10 h-10 text-orange-500 mx-auto" />
          <h2 class="text-lg font-bold text-gray-900 dark:text-white">Draft ready to start</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400">
            All {{ leagueDetailStore.currentLeague?.participants_count }} members have joined. You can activate the draft now.
          </p>
          <ButtonComponent
            variant="primary"
            size="sm"
        ...
        <div
          ...
          class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700/60 px-5 py-6 text-center space-y-3"
        >
          <v-icon name="hi-solid-user-group" class="w-10 h-10 text-blue-500 mx-auto" />
          <h2 class="text-lg font-bold text-gray-900 dark:text-white">Waiting for members</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400">
            {{ leagueDetailStore.currentLeague?.members_count || 0 }} of
            {{ leagueDetailStore.currentLeague?.participants_count }} members have joined.
            The draft can be activated once all members are present.
          </p>
          <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 max-w-xs mx-auto">
            <div
        ...
        >
          ...
          <h2 class="text-lg font-bold text-gray-900 dark:text-white">Draft not available yet</h2>
          <p class="text-sm text-gray-500 dark:text-gray-400">
            The league admin hasn't activated the draft yet. You'll be notified when it starts.
          </p>
        </div>
      </template>
...
const membersProgress = computed(() => {
  ...
  return Math.round(((league.members_count || 0) / league.participants_count) * 100)
})

// Ably subscription for real-time member joins
const channel = fantasyLeagueChannel(fantasyLeagueUuid.value)
...
const handleActivateDraft = async () => {
  try {
    ...
    const updatedLeague = await fantasyLeagueService.showFantasyLeague(fantasyLeagueUuid.value)
