<template>
  <div class="flex items-center gap-1.5">
    ...
    <button
      ...
      class="shrink-0 p-1.5 rounded-full text-gray-400 dark:text-gray-500 active:text-amber-500 disabled:opacity-30 disabled:pointer-events-none transition-colors"
    >
    ...
    >
      ...
      >
        ...
        <span
          ...
          :class="i === modelValue ? 'bg-white' : 'bg-amber-400'"
        >
          <span
            class="absolute inset-0 rounded-full animate-ping"
            :class="i === modelValue ? 'bg-white/70' : 'bg-amber-400/60'"
          />
        </span>
      </button>
    </div>
...
const props = defineProps<{
  ...
  /** Index of the selected knockout stage (v-model). */
  modelValue: number;
}>();
...
const goTo = (index: number) => {
  if (index < 0 || index > props.stages.length - 1 || index === props.modelValue) return;
...
// --- Keep the selected stage centered ---
...
watch(() => props.stages, () => nextTick(centerSelected), { deep: false });
