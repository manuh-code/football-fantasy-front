    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden mb-6">
      ...
      >
        <div class="w-10 h-10 rounded-lg bg-amber-50 dark:bg-amber-900/30 flex items-center justify-center flex-shrink-0">
          <v-icon name="hi-solid-credit-card" class="w-5 h-5 text-amber-600 dark:text-amber-400" />
        </div>
        <div class="flex-1 text-left min-w-0">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $t('user.settings.menu.paymentMethods.title') }}</p>
          <p class="text-xs text-gray-500 dark:text-gray-400">{{ $t('user.settings.menu.paymentMethods.subtitle') }}</p>
        </div>
        <v-icon name="hi-solid-chevron-right" class="w-4 h-4 text-gray-400 dark:text-gray-500 flex-shrink-0" />
      </button>
