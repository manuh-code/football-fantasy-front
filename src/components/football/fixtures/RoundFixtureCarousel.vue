<template>
  <section class="w-full mt-6">
    ...
    <div v-else-if="rounds.length === 0" class="text-center py-8 text-gray-400 dark:text-gray-500">No rounds available.</div>

    <div v-else class="w-full">
      <!-- Rounds selector -->
      <div class="overflow-x-auto hide-scrollbar px-2 py-2" ref="roundsWrapper">
        <div class="flex gap-2 items-center">
          <button v-for="(r, i) in rounds" :key="r.name + i" @click="selectRound(i)"
            :class="[
            ...
            <div class="truncate max-w-[8rem] flex items-center justify-center">
              <span>Round {{ r.name }}</span>
              <span v-if="r.games_in_current_week" class="ml-2 inline-block w-2.5 h-2.5 rounded-full bg-emerald-300 dark:bg-emerald-400 relative">
                <span class="absolute inset-0 rounded-full animate-ping bg-emerald-300 dark:bg-emerald-400 opacity-50"></span>
              </span>
            </div>
          </button>
        </div>
      ...
      <div class="mt-4">
        <div v-if="selectedFixtures.length === 0" class="text-center py-8 text-gray-400 dark:text-gray-500">No fixtures for this round.</div>
        <div v-else ref="carousel" class="flex gap-4 overflow-x-auto scroll-smooth hide-scrollbar w-full px-1 md:px-6">
          <div v-for="(fixture, idx) in selectedFixtures" :key="idx"
            ...
            <div class="flex items-center gap-6 w-full justify-center">
              ...
              <div class="flex flex-col items-center">
                <img :src="(fixture.participants[1]?.image_path) || '/img/default-avatar.svg'" :alt="fixture.participants[1]?.name || 'Team 2'" class="w-12 h-12 rounded-full border-2 border-gray-200 dark:border-gray-700" />
                <span :class="resultClass(fixture, 1)" class="mt-2 text-sm text-center max-w-[90px] truncate">{{ fixture.participants[1]?.name || 'Team 2' }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="flex flex-col items-center mt-4 w-full">
        <div class="flex items-center gap-4">
          ...
          <div class="flex gap-2">
            <span v-for="(f, i) in selectedFixtures" :key="i"
              :class="[
                currentCard === i
...
const getScore = (fixture: FootballFixtureResponse, participantIdx: number) => {
  ...
  const s = fixture.scores.find((sc: import('@/interfaces/football/fixture/ScoreResponse').ScoreResponse) => sc.score?.participant === (participantIdx === 0 ? 'home' : 'away'))
  if (s && typeof s.score?.goals === 'number') return s.score.goals
  }
  return null
}

const resultClass = (fixture: FootballFixtureResponse, participantIdx: number) => {
  const participant = fixture.participants?.[participantIdx]
...
const fetchRounds = async () => {
  ...
  try {
    const res = await FootballFixtureService.getRoundBySeasonFixtures(seasonUuid)
...
const centerRound = (idx: number) => {
  ...
  const button = wrapper.querySelectorAll('button')[idx] as HTMLElement | undefined
  if (!button) return
  const wrapperRect = wrapper.getBoundingClientRect()
  const btnRect = button.getBoundingClientRect()
  const targetScroll = wrapper.scrollLeft + (btnRect.left - wrapperRect.left) - (wrapperRect.width / 2 - btnRect.width / 2)
...
const scrollRight = () => {
  if (selectedFixtures.value.length === 0) return
  let newIdx = currentCard.value + 1
  if (newIdx > selectedFixtures.value.length - 1) newIdx = selectedFixtures.value.length - 1
  scrollToCard(newIdx)
}

const scrollToCard = (idx: number) => {
  if (!carousel.value) return
  const card = (carousel.value as HTMLElement).children[idx] as HTMLElement | undefined
  if (!card) return

  const carouselRect = (carousel.value as HTMLElement).getBoundingClientRect()
  const cardRect = card.getBoundingClientRect()

  const targetScroll: number = (carousel.value as HTMLElement).scrollLeft + (cardRect.left - carouselRect.left) - (carouselRect.width / 2 - cardRect.width / 2)
  ;(carousel.value as HTMLElement).scrollTo({ left: targetScroll, behavior: 'smooth' })
...
@media (prefers-reduced-motion: reduce) {
  .round-selected { transform: none !important; }
