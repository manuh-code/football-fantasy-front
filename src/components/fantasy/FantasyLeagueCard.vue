<template>
    <div 
        ...
        <div class="relative h-40 overflow-hidden">
            ...
            <!-- Status Badge -->
            <div v-if="league.isAdmin || league.isMember" class="absolute top-3 right-3">
                <span v-if="league.isAdmin" 
                    class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-gradient-to-r from-yellow-400 to-orange-500 text-white shadow-lg">
                    <v-icon name="hi-solid-star" class="w-3.5 h-3.5 mr-1.5" />
                    Admin
                </span>
                <span v-else
                    class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-gradient-to-r from-blue-500 to-indigo-600 text-white shadow-lg">
                    <v-icon name="hi-solid-check-circle" class="w-3.5 h-3.5 mr-1.5" />
                    Member
                </span>
            </div>
        ...
        <div class="p-5">
            <!-- Description -->
            <p class="text-gray-600 dark:text-gray-400 text-sm mb-4 line-clamp-2 leading-relaxed min-h-[2.5rem]">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
                <div class="bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/20 dark:to-emerald-800/20 rounded-lg p-3 text-center border border-emerald-200 dark:border-emerald-700/50">
                    <p class="text-2xl font-bold text-emerald-700 dark:text-emerald-300 mb-1">{{ league.participants_count }}</p>
                ...
                <div class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-800/20 rounded-lg p-3 text-center border border-blue-200 dark:border-blue-700/50">
                    <p class="text-2xl font-bold text-blue-700 dark:text-blue-300 mb-1">{{ league.members_count }}</p>
                    <p class="text-xs text-blue-600 dark:text-blue-400 font-medium">Members</p>
                </div>
            </div>

            <!-- Join Button (only if not a member and showJoinButton is true) -->
            <ButtonComponent
            ...
            >
                <v-icon name="hi-solid-user-add" class="w-4 h-4 mr-2" />
